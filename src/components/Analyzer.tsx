  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-2">
        <h1 className="text-[40px] font-semibold tracking-[-0.03em] leading-tight text-[#1A1A1A]">How secure is your password?</h1>
      ...
      <div className="space-y-4">
        <div className="relative group">
          <input
            ...
            className="w-full h-14 pl-4 pr-24 bg-white border border-[#E5E7EB] rounded-xl text-lg font-mono focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all text-[#1A1A1A]"
          />
          <div className="absolute right-2 top-2 bottom-2 flex gap-1 items-center">
            {password && (
              <button
                type="button"
                onClick={() => setPassword('')}
                className="px-3 text-xs font-semibold text-[#6B7280] hover:bg-gray-50 rounded-md transition-colors uppercase tracking-wider"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {/* Security Checks */}
        <div className="space-y-4">
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 pt-2">
        ...
        <div className="space-y-6">
          <div className="p-6 bg-white border border-[#E5E7EB] rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.03)]">
            <p className="text-[10px] font-bold tracking-[0.1em] text-[#6B7280] uppercase mb-1">Estimated Crack Resistance</p>
            <h3 className="text-2xl font-semibold text-[#1A1A1A]">{analysis.crackTimeStr}</h3>
            <p className="text-[12px] text-[#9CA3AF] mt-2 leading-relaxed">{analysis.crackTimeDesc}</p>
          </div>
          ...
          <div className="p-6 bg-white border border-[#E5E7EB] rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.03)]">
            ...
            {breachCount === null && !isCheckingBreach && (
              <button 
                ...
                disabled={!password}
                className="w-full py-2.5 bg-white border border-[#E5E7EB] text-[13px] font-semibold text-[#1A1A1A] rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
...
function CheckItem({ met, text }: { met: boolean; text: string }) {
  return (
    <li className={`flex items-center gap-2 text-[13px] font-medium transition-colors duration-300 ${met ? 'text-green-600' : 'text-gray-400'}`}>
      <CheckCircle2 className={`w-4 h-4 shrink-0 transition-colors duration-300 ${met ? 'text-green-600' : 'text-gray-300'}`} />
      <span>{text}</span>
    </li>
  );
}
